...
(defmacro symbol-cache-no (symbol mask)
  (if (and (constantp symbol)
	   (constantp mask))
      `(load-time-value  
        (logand (ash (SB-KERNEL:GET-LISP-OBJ-ADDRESS ,symbol) -2) ,mask))
      `(logand (ash (the fixnum (SB-KERNEL:GET-LISP-OBJ-ADDRESS ,symbol)) -2) ,mask)))

(defmacro object-cache-no (object mask)
  `(logand (the fixnum (SB-KERNEL:GET-LISP-OBJ-ADDRESS ,object)) ,mask))

  ;;   
;;;;;; printing-random-thing-internal
  ;;
(defun printing-random-thing-internal (thing stream)
  (format stream "~O" (SB-KERNEL:GET-LISP-OBJ-ADDRESS thing)))
